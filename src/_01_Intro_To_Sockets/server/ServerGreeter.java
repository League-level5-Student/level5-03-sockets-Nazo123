...
ServerSocket a;
	public ServerGreeter() throws IOException {
		...
		//   you must define the port at which the server will listen for connections.
		 a = new ServerSocket(21);
		//*OPTIONAL* you can set a time limit for the server to wait by using the 
		//  ServerSocket's setSoTimeout(int timeInMilliSeconds) method
		a.setSoTimeout(20000000);
	}
	...
	public void run() {
		...
		JOptionPane.showMessageDialog(null, "Waiting to connect to server");
...
DataInputStream str = new DataInputStream(ar.getInputStream());

				//12. Print the message from the DataInputStream object using the readUTF() method
JOptionPane.showMessageDialog(null,str.readUTF());
				...
				DataOutputStream stre = new DataOutputStream(ar.getOutputStream());
				//14. Use the DataOutputStream object to send a message to the client using the writeUTF(String message) method.
				stre.writeUTF(JOptionPane.showInputDialog("Send a message"));
			...
			System.out.println("There was an SocketTimeoutException");
			test = false;
